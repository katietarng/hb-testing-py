...
def is_mel(name, email):
    ...
    Using name or email address to test if Mel is responding:

    >>> is_mel('Christina', 'mel@ubermelon.com')
    True
    >>> is_mel('Christina', 'c@gmail.com')
    False
...
def most_and_least_common_type(treats):
    ...
    List of various treats returns most and least
    ...
    List of only one type of treat, returns that one type twice

    >>> treats = [{'type': 'dessert'},{'type': 'dessert'},{'type': 'dessert'}]
    >>> most_and_least_common_type(treats)
    ('dessert', 'dessert')

    List of treats that have a tie for most or least; returns most count for first type in the treats list

    >>> treats = [{'type': 'dessert'}, {'type': 'dessert'},  {'type': 'appetizer'},  {'type': 'appetizer'}, {'type': 'drink'}]
    >>> most_and_least_common_type(treats)
    ('dessert', 'drink')

    Empty list of treats; returns None twice
